...
V3_REC="대화 프로픔트 모음집/회상치료_세션기록_실행_프롬프트_v3.md"
V1_GUARDIAN="대화 프로픔트 모음집/보호자_리포트_생성_프롬프트_v1.md"

for f in "$V8" "$V2_REC" "$V3_REC" "$V1_GUARDIAN"; do
  if [ ! -f "$f" ]; then
...
python3 - "$V8" "$V2_REC" "$V3_REC" "$V1_GUARDIAN" > "$OUT" <<'PY'
import json, sys
v8 = open(sys.argv[1], encoding="utf-8").read()
v2 = open(sys.argv[2], encoding="utf-8").read()
v3 = open(sys.argv[3], encoding="utf-8").read()
v1_guardian = open(sys.argv[4], encoding="utf-8").read()
...
print()
print("// v2 는 v3 의 이전 버전. 새 호출은 V3 사용. v2 는 reference 로만 유지.")
print("export const SESSION_RECORD_SYSTEM_PROMPT_V2 =")
print("  " + json.dumps(v2, ensure_ascii=False) + ";")
print()
print("// 세션 종료 후 nextSessionPrep + dataPipeline 두 부분을 함께 생성.")
print("// 보호자 리포트는 이 V3 의 nextSessionPrep + sensitiveContent 를 입력으로 받는다.")
print("export const SESSION_RECORD_SYSTEM_PROMPT_V3 =")
